    ...
    <div className="w-full space-y-6 animate-in fade-in zoom-in-95 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className="flex items-center gap-4 bg-white/80 dark:bg-white/5 border border-slate-100 dark:border-white/10 p-5 rounded-3xl shadow-sm dark:shadow-none backdrop-blur-md transition-all hover:bg-white dark:hover:bg-white/10">
            <div className="p-3 bg-blue-500/10 rounded-2xl">
              <item.icon className="w-5 h-5 text-blue-600 dark:text-blue-400" />
            </div>
            <div>
              <p className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider">{item.label}</p>
              <p className="text-[10px] font-medium text-slate-500 dark:text-white/50">{item.desc}</p>
            </div>
          </div>
        ))}
      </div>

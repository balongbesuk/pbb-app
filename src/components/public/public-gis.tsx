...
export function PublicGis({ tahunPajak }: PublicGisProps) {
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in zoom-in-95 duration-700">
      <div className="relative w-full h-[650px] overflow-hidden rounded-[2.5rem] border border-slate-300 dark:border-white/10 bg-white dark:bg-black/40 shadow-xl dark:shadow-2xl backdrop-blur-sm">
        ...
        <div className="absolute bottom-10 left-6 z-[1000] max-w-[200px] hidden lg:block">
            <div className="bg-white dark:bg-[#050505] backdrop-blur-md p-4 rounded-3xl border border-slate-300 dark:border-white/10 shadow-xl dark:shadow-2xl space-y-3">
                <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400">
                    <MousePointer2 className="w-4 h-4" />
                    <span className="text-[10px] font-black uppercase tracking-widest">Panduan</span>
                </div>
                <p className="text-[10px] leading-relaxed text-slate-700 dark:text-white/60 font-medium italic">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className="flex items-center gap-4 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-5 rounded-3xl shadow-md dark:shadow-none backdrop-blur-md transition-all hover:shadow-lg hover:border-blue-200 dark:hover:bg-white/10">
            <div className="p-3 bg-blue-500/15 dark:bg-blue-500/10 rounded-2xl">
              <item.icon className="w-5 h-5 text-blue-600 dark:text-blue-400" />
            </div>
            <div>
              <p className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider">{item.label}</p>
              <p className="text-[10px] font-medium text-slate-600 dark:text-white/50">{item.desc}</p>
            </div>
          </div>
        ))}
      </div>

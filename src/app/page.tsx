...
export default async function IndexPage() {
  ...
  return (
    <div className="relative min-h-screen gradient-bg flex flex-col items-center pt-24 pb-12 px-6 sm:px-10 font-sans selection:bg-primary/20">
      <nav className="fixed top-0 w-full p-4 sm:p-6 flex justify-between items-center z-50">
        <div className="flex items-center gap-3 bg-white/50 dark:bg-zinc-950/50 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 shadow-sm transition hover:shadow-md">
          {config?.logoUrl ? (
            <Image
              src={config.logoUrl}
              alt={`Logo ${namaDesa}`}
          ...
          <span className="font-bold text-sm tracking-tight text-foreground dark:text-zinc-200">
            {namaDesa}
          </span>
        </div>
        ...
        <Link
          href="/login"
          className="group flex items-center gap-2 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-5 py-2.5 rounded-full font-bold text-xs uppercase tracking-widest shadow-lg shadow-zinc-900/20 hover:scale-105 active:scale-95 transition-all"
        >
          Masuk Admin
      ...
      <main className="w-full max-w-4xl flex-1 flex flex-col items-center justify-center gap-12 mt-12 sm:mt-20">
        <div className="text-center space-y-4 max-w-2xl px-4 animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black text-gray-900 dark:text-white tracking-tight leading-[1.1]">
            Cek Tagihan & Bayar <span className="text-primary block sm:inline mt-1 sm:mt-0 px-2 bg-gradient-to-r from-emerald-400 to-teal-600 bg-clip-text text-transparent pb-2">PBB</span> Lebih Mudah.
          </h1>
      ...
      <footer className="mt-auto pt-24 text-center pb-6 opacity-60 hover:opacity-100 transition-opacity">
        <p className="text-xs font-bold uppercase tracking-widest mb-1.5"><span className="text-primary">PBB Manager</span> &copy; {new Date().getFullYear()}</p>
        <p className="text-[10px] font-medium text-muted-foreground">Pemerintah Desa {namaDesa}</p>
      </footer>
    </div>
  );
}

...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!dbEntry) {
            return NextResponse.json({ error: "Tidak ada file database (.db) di dalam ZIP." }, { status: 400 });
        }
        ...
        return NextResponse.json({ success: true, message: "Database berhasil dipulihkan dari cadangan. Silakan login kembali." });
    } catch (error: any) {
        console.error("Restore Error:", error);
        return NextResponse.json({ error: "Gagal memulihkan database: " + error.message }, { status: 500 });
    }
}

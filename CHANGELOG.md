...
- **Anti-Scraping & Robot Protection (2026-05-22)**: Menambahkan konfigurasi anti-scraping lengkap untuk mencegah bot, crawler, mesin pencari, dan bot AI scraper (seperti GPTBot, ClaudeBot, CommonCrawl, dll.) merayapi atau mengambil data wajib pajak dari aplikasi. Proteksi diimplementasikan ganda via berkas `public/robots.txt` dengan aturan global `Disallow: /`, serta injeksi tag meta `robots` (`noindex, nofollow, nocache`) secara global melalui Next.js Metadata Engine pada `layout.tsx`.
